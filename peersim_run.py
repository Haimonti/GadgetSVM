...
`src/network_layer/peersim_python/` instead of the p2pfl runtime — so it needs
...
from evaluation.visualizer import (
    plot_loss_vs_time, plot_gap_vs_time, plot_comm_cost_vs_time,
)

from src.network_layer.peersim_python.core import (
...
from src.network_layer.peersim_python.observers import (
...
def run(cycles: int = None) -> None:
    ...
    initializers = [
        ...
        DataInitializer(
            SDCA_PID, worker_data, CONFIG["LAMBDA"], CONFIG["T0_FRACTION"], CONFIG["GOSSIP_K"]
        ),
    ]
    ...
    # Plots — one graph each, all workers overlaid (same as main.py)
