...
from src.network_layer.peersim_python.core import Control, Network
from src.network_layer.peersim_python.cdsim import CDState
...
class DataInitializer(Control):
    ...
    def __init__(self, pid, worker_data, lambda_reg, t0_fraction, gossip_k):
        self.pid = pid
    ...
    def execute(self):
        for i, wd in enumerate(self.worker_data):
            ...
            logger.info("init", f"Node {i} loaded — {wd['n_local']} local samples")
        return False
